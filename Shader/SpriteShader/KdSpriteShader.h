...
public:
	...
	struct Vertex
	{
		Math::Vector3 Pos;
		Math::Vector2 UV;
		Math::Vector3 ZCalcTex;
	};
	//enum ShaderTypes(int型)
	//通常の列挙型と違い型を指定する必要がある
	//保守性を上げる
	enum class ShaderTypes	//(ShaderTypes型)
	{
		Basic,
		Blur,
		Brightness,
	...
	void Begin(ShaderTypes type = ShaderTypes::Basic, bool linear = false, bool disableZBuffer = true);
	...
	void SetBlurDir(const Math::Vector2& _dir)
	{
		m_cb2.Work().BlurDir = _dir;
	}
	void SetBlurPower(const int& _power)
	{
		m_cb2.Work().BlurPower = _power;
	}

	void SetEdgeParamEnable(bool _enable)
	{
		m_cb4.Work().EdgeParamEnable = _enable;
	}

	void SetEdgeParam(float _texel, float _value, float _pow)
	{
		m_cb4.Work().texel = _texel;
		m_cb4.Work().value = _value;
		m_cb4.Work().pow = _pow;
	}
private:
	...
	ID3D11PixelShader*	    m_BlurPS = nullptr;			// ピクセルシェーダー(ブラー)
	ID3D11PixelShader*	    m_BrightnessPS = nullptr;			// ピクセルシェーダー(ハイブリッド)
	ID3D11PixelShader*		m_EdgePS = nullptr;			// ピクセルシェーダー(エージ)
	...
	struct cbBlur {
		//8bit
		Math::Vector2		BlurDir = { 0,0 };
		int					BlurPower = 0;
		//8bit
		float				tmp;
	};
	KdConstantBuffer<cbBlur>	m_cb2;

	struct cbBright{
		//8bit
		Math::Vector2		BrightPow = { 0,0 };
		//8bit
		float				tmp[2];
	};
	KdConstantBuffer<cbBright>	m_cb3;

	struct cbEdge {
		int					EdgeParamEnable = 0;
		float				texel = 1;
		float				value = 0.0001;
		float				pow = 1;
	};
	KdConstantBuffer<cbEdge>	m_cb4;

...
// 頂点シェーダー作成
	...
	{
		// コンパイル済みのシェーダーヘッダーファイルをインクルード
		#include "KdSpriteShader_BlurPS.inc"
	...
	{
		// コンパイル済みのシェーダーヘッダーファイルをインクルード
		#include "KdSpriteShader_HighBrightnessPS.inc"

		if (FAILED(D3D.GetDev()->CreatePixelShader(compiledBuffer, sizeof(compiledBuffer), nullptr, &m_BrightnessPS))) {
			assert(0 && "ピクセルシェーダー作成失敗");
			Release();
			return false;
		}
	...
	{
		// コンパイル済みのシェーダーヘッダーファイルをインクルード
		#include "KdSpriteShader_EdgePS.inc"

		if (FAILED(D3D.GetDev()->CreatePixelShader(compiledBuffer, sizeof(compiledBuffer), nullptr, &m_EdgePS))) {
			assert(0 && "ピクセルシェーダー作成失敗");
			Release();
			return false;
		}
	...
	m_ds = D3D.CreateDepthStencilState(true, true);
...
void KdSpriteShader::Release()
{
	...
	KdSafeRelease(m_BrightnessPS);
	KdSafeRelease(m_EdgePS);
...
void KdSpriteShader::Begin(ShaderTypes type, bool linear, bool disableZBuffer)
{
	// 既にBeginしている
	if (m_isBegin)return;
	m_isBegin = true;
	...
	switch (type)
	{
	case ShaderTypes::Basic:
		D3D.GetDevContext()->PSSetShader(m_PS, 0, 0);
		break;
	case ShaderTypes::Blur:
		D3D.GetDevContext()->PSSetShader(m_BlurPS, 0, 0);
		break;
	case ShaderTypes::Brightness:
		D3D.GetDevContext()->PSSetShader(m_BrightnessPS, 0, 0);
		break;
	case ShaderTypes::Edge:
		D3D.GetDevContext()->PSSetShader(m_EdgePS, 0, 0);
		break;
	}
	...
	if (type == ShaderTypes::Blur)
	{
		//確認する
		m_cb2.Write();
		D3D.GetDevContext()->VSSetConstantBuffers(2, 1, m_cb2.GetAddress());
		D3D.GetDevContext()->PSSetConstantBuffers(2, 1, m_cb2.GetAddress());
	}
	if (type == ShaderTypes::Brightness)
	{
		//確認する
		m_cb3.Write();
		D3D.GetDevContext()->VSSetConstantBuffers(3, 1, m_cb3.GetAddress());
		D3D.GetDevContext()->PSSetConstantBuffers(3, 1, m_cb3.GetAddress());
	}
	if (type == ShaderTypes::Edge)
	{
		//確認する
		m_cb4.Write();
		D3D.GetDevContext()->VSSetConstantBuffers(4, 1, m_cb4.GetAddress());
		D3D.GetDevContext()->PSSetConstantBuffers(4, 1, m_cb4.GetAddress());
	}
}
